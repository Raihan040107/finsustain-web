    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="border border-white/[0.06] rounded-2xl p-16 text-center bg-white/[0.01]">
          ...
          <p className="text-[#b0a89e] text-sm mb-1">{searchQuery ? `Tidak ditemukan FAQ untuk "${searchQuery}"` : "Belum ada FAQ."}</p>
          {!searchQuery && <p className="text-[#b0a89e]/50 text-xs mb-5">Tambahkan FAQ pertama untuk ditampilkan di halaman publik.</p>}
          {!searchQuery && (
          ...
          {filtered.map((faq, index) => (
            <div
              ...
              className={`group relative border rounded-2xl p-4 flex items-start gap-4 transition-all cursor-grab active:cursor-grabbing active:scale-[0.99] active:shadow-2xl
                ...
                ${faq.is_active ? "bg-[#2d2d2d] border-[#404040] hover:border-[#e05c2a]/40 hover:bg-[#333333]" : "bg-[#262626] border-[#383838] opacity-75"}`}
            >
              {/* Drag handle */}
              ...
              {/* Nomor urut */}
              <div className="shrink-0 w-6 h-6 rounded-lg bg-[#e05c2a]/10 border border-[#e05c2a]/20 flex items-center justify-center text-[#e05c2a] text-[10px] font-bold mt-0.5">{index + 1}</div>

              {/* Konten */}
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-2 flex-wrap mb-1">
                  <p className="font-semibold text-sm text-white leading-snug flex-1 min-w-0">{faq.question}</p>
                  {!faq.is_active && <span className="text-[10px] font-bold bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full shrink-0 mt-0.5">Nonaktif</span>}
                </div>
          ...
          <div className="w-full max-w-lg bg-[#2a2a2a] border border-white/[0.09] rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="font-semibold text-sm text-white">{modalMode === "add" ? "Tambah FAQ Baru" : "Edit FAQ"}</h2>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-[#2a2a2a] border border-white/[0.09] rounded-2xl shadow-2xl p-6 text-center">
            ...
            <p className="text-sm text-white font-medium mb-6 px-2 line-clamp-3 bg-white/[0.03] border border-white/[0.06] rounded-xl py-3 leading-relaxed">"{deleteTarget.question}"</p>
